/* SODB V70.1.1 - Điểm danh lớp chính khóa + Sổ nhóm Chuyên đề / GDTC
...
function syncAttendanceModeUiV7011(){
  ...
  if(tools)tools.classList.toggle('d-none',complete);
  if(listWrap)listWrap.classList.toggle('d-none',complete);
  const nameEl=document.getElementById('tenHSVang');
  if(nameEl&&!nameEl.value)nameEl.placeholder=complete?'Điểm danh đủ — không có học sinh vắng':'Tích học sinh vắng trong danh sách phía trên';
}
...
function syncGroupAbsenceSummaryV6951(){
  ...
  const count=complete?0:groupAbsentIdsV6951.size;
  const selected=complete?[]:groupRosterV6951.filter(x=>groupAbsentIdsV6951.has(String(x.maHS||'')));
...
function renderGroupAttendanceV6951(){
  ...
  if(!groupRosterV6951.length){
    wrap.innerHTML='<div class="text-center text-muted py-3">Chọn lớp để tải danh sách học sinh. Nếu đã chọn lớp nhưng chưa có dữ liệu, Admin cần import danh sách học sinh vào Supabase.</div>';
...
function toggleGroupAbsentV6951(encodedId,checked){
  const id=decodeURIComponent(String(encodedId||''));if(!id||isAttendanceCompleteV701())return;
...
async function refreshGroupAttendanceV6951(preserveIds){
  ...
  if(!lop){if(panel)panel.classList.remove('d-none');groupRosterV6951=[];groupAbsentIdsV6951.clear();groupRosterKeyV6951='';groupRosterModeV701='';if(countEl)countEl.value='0';if(nameEl){nameEl.value='';nameEl.placeholder='Chọn lớp để điểm danh';}if(metaEl)metaEl.textContent='Chọn lớp để tải danh sách học sinh từ Supabase.';renderGroupAttendanceV6951();return;}
  if(panel)panel.classList.remove('d-none');if(countEl)countEl.readOnly=true;if(nameEl)nameEl.readOnly=true;
  if(!gvbmDangNhapInfo?.sessionToken){if(metaEl)metaEl.textContent='Cần phiên giáo viên bộ môn để tải danh sách học sinh.';return;}
  const isGroup=isGroupBookClassV6951(lop);if(isGroup&&!date){if(metaEl)metaEl.textContent='Chọn ngày dạy để tải đúng danh sách thành viên nhóm đang hiệu lực.';return;}
  const mode=isGroup?'GROUP':'LOP_CHINH',key=`${mode}|${lop}|${isGroup?date:''}`;
  ...
  if(completeEl){if(preserveArray)completeEl.checked=preserveArray.length===0;else if(key!==groupRosterKeyV6951)completeEl.checked=true;}
  groupRosterModeV701=mode;
  if(metaEl)metaEl.textContent=isGroup?'Đang tải danh sách học sinh nhóm từ Supabase...':'Đang tải danh sách học sinh lớp từ Supabase...';
  ...
  try{
    const method=isGroup?'layDanhSachHocSinhNhomV6951':'layDanhSachHocSinhLopV701';
    const r=await callSodbEdgeRpcV67(method,[lop,date,{token:gvbmDangNhapInfo.sessionToken}]);
    ...
    if(metaEl){
      if(isGroup)metaEl.textContent=`${r.loaiNhom==='GDTC'?'GDTC':'Chuyên đề'} ${lop}${r.subject?' · '+r.subject:''} · danh sách theo ngày ${date.split('-').reverse().join('/')}.`;
      else metaEl.textContent=`Lớp ${lop} · danh sách học sinh chính khóa trên Supabase${groupRosterV6951.length?'':' (chưa có dữ liệu)'}.`;
    }
    renderGroupAttendanceV6951();
  }catch(e){
    if(requestSeq!==groupRosterRequestSeqV6951)return;groupRosterV6951=[];groupAbsentIdsV6951.clear();groupRosterKeyV6951=key;
    if(metaEl)metaEl.textContent=e?.message||String(e);renderGroupAttendanceV6951();
  }
}
function resetGroupAttendanceAfterSaveV6951(){
  groupAbsentIdsV6951.clear();const complete=document.getElementById('attendanceCompleteV701');if(complete)complete.checked=true;renderGroupAttendanceV6951();
}
