...
function v702BuildArchivePrintHtml(res){
  ...
  days.forEach((day,di)=>{for(let i=0;i<10;i++){const buoi=i<5?'Sang':'Chieu',tiet=i<5?i+1:i-4,a=by.get(`${day}|${buoi}|${tiet}`)||[];
    const join=(field,sep='<br>')=>a.map(x=>v700Esc(x[field]??'')).filter(Boolean).join(sep);
  ...
  <div class="footer"><div class="box"><b>Tổng kết tuần:</b> ${v700Esc(res.summary?.rows||0)} bản ghi · Tổng lượt vắng: ${v700Esc(res.summary?.absent||0)} · ĐTB: ${v700Esc(res.summary?.avg||0)} · Chưa có ảnh chữ ký: ${v700Esc(res.summary?.missingSignatures||0)}<br><b>Ý kiến GVCN/GV phụ trách:</b> ${v700Esc(gvcn.ykien||'')}</div><div class="box sign"><b>GVCN / GV PHỤ TRÁCH</b>${gvcnUrl?`<img src="${v700Esc(normalizeSignatureUrlV67_1(gvcnUrl))}">`:'<div class="nosig">Chưa có ảnh chữ ký</div>'}<div><b>${v700Esc(gvcn.tenGVCN||'')}</b></div></div><div class="box sign"><b>BAN GIÁM HIỆU</b>${bghUrl?`<img src="${v700Esc(normalizeSignatureUrlV67_1(bghUrl))}">`:'<div class="nosig">Chưa có ảnh chữ ký</div>'}<div><b>${v700Esc(bgh.tenBGH||'')}</b></div></div></div></body></html>`;
}
