  ...
  async function uploadHSV4(){
    ...
    setBusyV13(true,'Đang cập nhật danh sách học sinh trên Supabase...');
    try{
      const res=await callSodbEdgeRpcV67('capNhatDanhSachHocSinhV701',[parsedHSDataV4,document.getElementById('hsModeV4').value,{token:adminDangNhapInfo.sessionToken}]);
      setBusyV13(false);if(!res?.success){alertV13('❌ '+(res?.message||'Không lưu được danh sách học sinh.'));return;}
      const p=document.getElementById('hsPreviewV4');if(p)p.innerText=`Năm học ${res.namHoc||''}: đã xử lý ${res.rows||0} học sinh trên Supabase.`;
      parsedHSDataV4=[];const input=document.getElementById('hsFileV4');if(input)input.value='';alertV13('✅ '+(res.message||'Đã cập nhật danh sách học sinh Supabase.'));
    }catch(err){setBusyV13(false);alertV13('❌ Không lưu được danh sách học sinh: '+(err?.message||err));}
  }
